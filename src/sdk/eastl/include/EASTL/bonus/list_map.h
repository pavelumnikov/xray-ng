...
namespace eastl
{
	...
	///
	template <typename Allocator, typename Key, typename T, typename Compare = eastl::less<Key>>
	class list_map
	...
	public:
		...
		typedef list_map<Allocator, Key, T, Compare>                                                            this_type;
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline list_map<Allocator, Key, T, Compare>::list_map(const allocator_type& allocator)
		: base_type(allocator)
	{
		mNode.mpNext = &mNode;
		mNode.mpPrev = &mNode;
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline list_map<Allocator, Key, T, Compare>::list_map(const Compare& compare, const allocator_type& allocator)
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::iterator 
	list_map<Allocator, Key, T, Compare>::begin() EA_NOEXCEPT
	{
		return iterator(mNode.mpNext);
	}

	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_iterator 
	list_map<Allocator, Key, T, Compare>::begin() const EA_NOEXCEPT
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_iterator 
	list_map<Allocator, Key, T, Compare>::cbegin() const EA_NOEXCEPT
	{
		return const_iterator(mNode.mpNext);
	}
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::iterator 
	list_map<Allocator, Key, T, Compare>::end() EA_NOEXCEPT
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_iterator 
	list_map<Allocator, Key, T, Compare>::end() const EA_NOEXCEPT
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_iterator 
	list_map<Allocator, Key, T, Compare>::cend() const EA_NOEXCEPT
	{
		return const_iterator(&mNode);
	}

	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::reverse_iterator 
	list_map<Allocator, Key, T, Compare>::rbegin() EA_NOEXCEPT
	{
		return reverse_iterator(&mNode);
	}

	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_reverse_iterator 
	list_map<Allocator, Key, T, Compare>::rbegin() const EA_NOEXCEPT
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_reverse_iterator 
	list_map<Allocator, Key, T, Compare>::crbegin() const EA_NOEXCEPT
	{
		return const_reverse_iterator(&mNode);
	}
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::reverse_iterator 
	list_map<Allocator, Key, T, Compare>::rend() EA_NOEXCEPT
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_reverse_iterator 
	list_map<Allocator, Key, T, Compare>::rend() const EA_NOEXCEPT
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_reverse_iterator 
	list_map<Allocator, Key, T, Compare>::crend() const EA_NOEXCEPT
	{
		return const_reverse_iterator(mNode.mpNext);
	}

	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::reference
	list_map<Allocator, Key, T, Compare>::front()
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_reference
	list_map<Allocator, Key, T, Compare>::front() const
	{
		#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
			// We allow the user to reference an empty container.
		#elif EASTL_ASSERT_ENABLED
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::reference
	list_map<Allocator, Key, T, Compare>::back()
	{
		#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
			// We allow the user to reference an empty container.
		#elif EASTL_ASSERT_ENABLED
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_reference
	list_map<Allocator, Key, T, Compare>::back() const
	{
		#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
			// We allow the user to reference an empty container.
		#elif EASTL_ASSERT_ENABLED
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	bool list_map<Allocator, Key, T, Compare>::push_front(const value_type& value)
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	bool list_map<Allocator, Key, T, Compare>::push_back(const value_type& value)
	{
		internal_value_type tempValue(value);
		typename base_type::insert_return_type baseReturn = base_type::insert(tempValue);
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	bool list_map<Allocator, Key, T, Compare>::push_front(const key_type& key, const mapped_type& value)
	{
		return push_front(eastl::make_pair(key, value));
	}

	template <typename Allocator, typename Key, typename T, typename Compare>
	bool list_map<Allocator, Key, T, Compare>::push_back(const key_type& key, const mapped_type& value)
	{
		return push_back(eastl::make_pair(key, value));
	}

	template <typename Allocator, typename Key, typename T, typename Compare>
	void list_map<Allocator, Key, T, Compare>::pop_front()
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	void list_map<Allocator, Key, T, Compare>::pop_back()
	{
		#if EASTL_ASSERT_ENABLED
			if (EASTL_UNLIKELY(empty()))
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::iterator 
	list_map<Allocator, Key, T, Compare>::find(const key_type& key)
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::const_iterator
	list_map<Allocator, Key, T, Compare>::find(const key_type& key) const
	{
	...
	template <typename U, typename Compare2>
	inline typename list_map<Allocator, Key, T, Compare>::iterator
	list_map<Allocator, Key, T, Compare>::find_as(const U& u, Compare2 compare2)
	{
	...
	template <typename U, typename Compare2>
	inline typename list_map<Allocator, Key, T, Compare>::const_iterator
	list_map<Allocator, Key, T, Compare>::find_as(const U& u, Compare2 compare2) const
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::size_type
	list_map<Allocator, Key, T, Compare>::count(const key_type& key) const
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::size_type
	list_map<Allocator, Key, T, Compare>::erase(const key_type& key)
	{
		typename base_type::iterator baseIter = base_type::find(key);
		if (baseIter != base_type::end())
		{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::iterator
	list_map<Allocator, Key, T, Compare>::erase(const_iterator position)
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	inline typename list_map<Allocator, Key, T, Compare>::reverse_iterator
	list_map<Allocator, Key, T, Compare>::erase(const_reverse_iterator position)
	{
		return reverse_iterator(erase((++position).base()));
	}

	template <typename Allocator, typename Key, typename T, typename Compare>
	void list_map<Allocator, Key, T, Compare>::clear()
	{
		base_type::clear();
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	void list_map<Allocator, Key, T, Compare>::reset_lose_memory()
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	bool list_map<Allocator, Key, T, Compare>::validate() const
	{
	...
	template <typename Allocator, typename Key, typename T, typename Compare>
	int list_map<Allocator, Key, T, Compare>::validate_iterator(const_iterator iter) const
	{
