...
static int createAnonymousFile(off_t size)
{
...
#endif
    {
        ...
        name = _glfwCalloc(strlen(path) + sizeof(template), 1);
        strcpy(name, path);
                               ...
                               struct wl_output *output)
{
    ...
    if (window->wl.monitorsCount + 1 > window->wl.monitorsSize)
    {
        ...
        window->wl.monitors =
            _glfwRealloc(window->wl.monitors,
                              ...
                              const _GLFWfbconfig* fbconfig)
{
    ...
    window->wl.monitors = _glfwCalloc(1, sizeof(_GLFWmonitor*));
...
void _glfwPlatformDestroyWindow(_GLFWwindow* window)
{
    ...
    _glfwFree(window->wl.title);
    _glfwFree(window->wl.monitors);
}
...
void _glfwPlatformSetClipboardString(const char* string)
{
    ...
    {
        _glfwFree(_glfw.wl.clipboardSendString);
...
static GLFWbool growClipboardString(void)
{
    ...
    clipboard = _glfwRealloc(clipboard, _glfw.wl.clipboardSize * 2);
    if (!clipboard)
    {
        _glfwInputError(GLFW_PLATFORM_ERROR,
