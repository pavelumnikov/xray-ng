                                  ...
                                  int hatCount)
{
    ...
    js->axes        = _glfwCalloc(axisCount, sizeof(float));
    js->buttons     = _glfwCalloc(buttonCount + hatCount * 4, 1);
    js->hats        = _glfwCalloc(hatCount, 1);
    js->axisCount   = axisCount;
    js->buttonCount = buttonCount;
    js->hatCount    = hatCount;

    _CRT_STRNCPY(js->guid, 33, guid, sizeof(js->guid) - 1);
...
GLFWAPI GLFWcursor* glfwCreateCursor(const GLFWimage* image, int xhot, int yhot)
{
    ...
    cursor = _glfwCalloc(1, sizeof(_GLFWcursor));
