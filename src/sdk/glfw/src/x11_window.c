...
static char** parseUriList(char* text, int* count)
{
    ...
    *count = 0;
    ...
    while ((line = strtok(text, "\r\n")))
    {
        ...
        char* path = _glfwCalloc(strlen(line) + 1, 1);
        paths = _glfwRealloc(paths, *count * sizeof(char*));
        paths[*count - 1] = path;
...
static char* convertLatin1toUTF8(const char* source)
{
    ...
    char* target = _glfwCalloc(size, 1);
    char* tp = target;
...
static void handleSelectionClear(XEvent* event)
{
    ...
    {
        _glfwFree(_glfw.x11.primarySelectionString);
        _glfw.x11.primarySelectionString = NULL;
    }
    else
    {
        _glfwFree(_glfw.x11.clipboardString);
    ...
    for (i = 0;  i < targetCount;  i++)
    {
        ...
        if (actualType == _glfw.x11.INCR)
        {
            ...
            for (;;)
            {
                ...
                {
                    size += itemCount;
                    string = _glfwRealloc(string, size);
        ...
        case KeyPress:
        {
            ...
            if (window->x11.ic)
            {
                ...
                if (!filtered)
                {
                    ...
                    if (status == XBufferOverflow)
                    {
                        chars = _glfwCalloc(count, sizeof(wchar_t));
                        count = XwcLookupString(window->x11.ic,
                                                &event->xkey,
                                                chars, count,
                                                NULL, &status);
                                ...
                                int count, const GLFWimage* images)
{
    if (count)
    {
        ...
        long* icon = _glfwCalloc(longCount, sizeof(long));
        long* target = icon;
